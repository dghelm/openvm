...
use openvm_hip_backend::{base::DeviceMatrix, prover_backend::HipBackend, types::F};
use openvm_hip_common::{copy::MemCopyH2D as _, d_buffer::DeviceBuffer};
use openvm_stark_backend::{prover::types::AirProvingContext, Chip};

use crate::{
    bitwise_op_lookup::{BitwiseOperationLookupChip, NUM_BITWISE_OP_LOOKUP_COLS},
    hip_abi::bitwise_op_lookup::tracegen,
};

pub struct BitwiseOperationLookupChipHIP<const NUM_BITS: usize> {
    pub count: Arc<DeviceBuffer<F>>,
    pub cpu_chip: Option<Arc<BitwiseOperationLookupChip<NUM_BITS>>>,
}

impl<const NUM_BITS: usize> BitwiseOperationLookupChipHIP<NUM_BITS> {
...
impl<const NUM_BITS: usize> Default for BitwiseOperationLookupChipHIP<NUM_BITS> {
    fn default() -> Self {
        Self::new()
    }
}

impl<RA, const NUM_BITS: usize> Chip<RA, HipBackend> for BitwiseOperationLookupChipHIP<NUM_BITS> {
