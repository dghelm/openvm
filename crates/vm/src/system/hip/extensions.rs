...
use openvm_circuit_primitives::{
    bitwise_op_lookup::{
        BitwiseOperationLookupAir, BitwiseOperationLookupChip, BitwiseOperationLookupChipHIP,
    },
    var_range::{VariableRangeCheckerAir, VariableRangeCheckerChip, VariableRangeCheckerChipHIP},
};
...
use super::{
    phantom::PhantomChipHIP, Poseidon2PeripheryChipHIP, SystemChipInventoryHIP, DIGEST_WIDTH,
};

/// A utility method to get the `VariableRangeCheckerChipHIP` from [ChipInventory].
/// Note, `VariableRangeCheckerChipHIP` always will always exist in the inventory.
pub fn get_inventory_range_checker(
    inventory: &mut ChipInventory<BabyBearPoseidon2Config, DenseRecordArena, HipBackend>,
) -> Arc<VariableRangeCheckerChipHIP> {
    inventory
        .find_chip::<Arc<VariableRangeCheckerChipHIP>>()
        .next()
        .unwrap()
        .clone()
}

/// A utility method to find a **byte** [BitwiseOperationLookupChipHIP] or create one and add
/// to the inventory if it does not exist.
pub fn get_or_create_bitwise_op_lookup(
    inventory: &mut ChipInventory<BabyBearPoseidon2Config, DenseRecordArena, HipBackend>,
) -> Result<Arc<BitwiseOperationLookupChipHIP<8>>, ChipInventoryError> {
    let bitwise_lu = {
        let existing_chip = inventory
            .find_chip::<Arc<BitwiseOperationLookupChipHIP<8>>>()
            .next();
        if let Some(chip) = existing_chip {
            ...
            let chip = Arc::new(BitwiseOperationLookupChipHIP::hybrid(Arc::new(
...
#[derive(Clone)]
pub struct SystemHipBuilder;

impl VmBuilder<HipBabyBearPoseidon2Engine> for SystemHipBuilder {
    ...
    > {
        ...
        let range_checker = Arc::new(VariableRangeCheckerChipHIP::hybrid(Arc::new(
        ...
        let hasher_chip = if config.continuation_enabled {
            ...
            let chip = Arc::new(Poseidon2PeripheryChipHIP::new(
                max_buffer_size,
                sbox_registers,
            ));
            inventory.add_periphery_chip(chip.clone());
        ...
        let system = SystemChipInventoryHIP::new(
            config,
        ...
        let phantom_chip = PhantomChipHIP::new();
