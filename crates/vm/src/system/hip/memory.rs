...
use openvm_hip_common::{
    copy::{hip_memcpy, MemCopyD2D, MemCopyH2D},
...
use super::{
    access_adapters::AccessAdapterInventoryHIP,
    boundary::{BoundaryChipHIP, BoundaryFields},
    merkle_tree::{MemoryMerkleTree, TIMESTAMPED_BLOCK_WIDTH},
    Poseidon2PeripheryChipHIP, DIGEST_WIDTH,
};

pub struct MemoryInventoryHIP {
    pub boundary: BoundaryChipHIP,
    pub access_adapters: AccessAdapterInventoryHIP,
    pub persistent: Option<PersistentMemoryInventoryHIP>,
    #[cfg(feature = "metrics")]
    pub(super) unpadded_merkle_height: usize,
}

pub struct PersistentMemoryInventoryHIP {
    pub merkle_tree: MemoryMerkleTree,
    pub initial_memory: Vec<DeviceBuffer<u8>>,
}

impl MemoryInventoryHIP {
    pub fn volatile(config: MemoryConfig, range_checker: Arc<VariableRangeCheckerChipHIP>) -> Self {
        let addr_space_max_bits = log2_ceil_usize(
            (ADDR_SPACE_OFFSET + 2u32.pow(config.addr_space_height as u32)) as usize,
        );
        Self {
            boundary: BoundaryChipHIP::volatile(
                range_checker.clone(),
                addr_space_max_bits,
                config.pointer_max_bits,
            ),
            access_adapters: AccessAdapterInventoryHIP::new(
                range_checker,
    ...
    pub fn persistent(
        ...
        hasher_chip: Arc<Poseidon2PeripheryChipHIP>,
    ) -> Self {
        Self {
            boundary: BoundaryChipHIP::persistent(hasher_chip.shared_buffer()),
            access_adapters: AccessAdapterInventoryHIP::new(
                range_checker,
                config.max_access_adapter_n,
                config.timestamp_max_bits,
            ),
            persistent: Some(PersistentMemoryInventoryHIP {
...
impl Drop for PersistentMemoryInventoryHIP {
    fn drop(&mut self) {
