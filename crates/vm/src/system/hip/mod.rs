...
use connector::VmConnectorChipHIP;
use memory::MemoryInventoryHIP;
use openvm_circuit::{
...
use openvm_hip_backend::{prover_backend::HipBackend, types::F};
use openvm_stark_backend::{
    prover::types::{AirProvingContext, CommittedTraceData},
    Chip,
};
use openvm_stark_sdk::config::baby_bear_poseidon2::BabyBearPoseidon2Config;
use poseidon2::Poseidon2PeripheryChipHIP;
use program::ProgramChipHIP;
use public_values::PublicValuesChipHIP;
...
pub struct SystemChipInventoryHIP {
    pub program: ProgramChipHIP,
    pub connector: VmConnectorChipHIP,
    pub memory_inventory: MemoryInventoryHIP,
    pub public_values: Option<PublicValuesChipHIP>,
}

impl SystemChipInventoryHIP {
    pub fn new(
        config: &SystemConfig,
        mem_inventory: &MemoryAirInventory<BabyBearPoseidon2Config>,
        range_checker: Arc<VariableRangeCheckerChipHIP>,
        hasher_chip: Option<Arc<Poseidon2PeripheryChipHIP>>,
    ) -> Self {
        ...
        let program_chip = ProgramChipHIP::new();
        let connector_chip = VmConnectorChipHIP::new(VmConnectorChip::new(
        ...
        let memory_inventory = match &mem_inventory.interface {
            ...
            MemoryInterfaceAirs::Volatile { .. } => {
                assert!(!config.continuation_enabled);
                MemoryInventoryHIP::volatile(config.memory_config.clone(), range_checker.clone())
            }
        };

        let public_values_chip = config.has_public_values_chip().then(|| {
            PublicValuesChipHIP::new(
                range_checker,
...
impl SystemChipComplex<DenseRecordArena, HipBackend> for SystemChipInventoryHIP {
    fn load_program(&mut self, cached_program_trace: CommittedTraceData<HipBackend>) {
    ...
    fn finalize_trace_heights(&self, heights: &mut [usize]) {
        use crate::system::hip::boundary::BoundaryFields;
